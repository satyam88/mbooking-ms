pipeline {
    ...
    environment {
        ...
        IMAGE_TAG = "${BRANCH_NAME}-mbooking-ms-v.1.${BUILD_NUMBER}"
        DEV_IMAGE_TAG = "dev-mbooking-ms-v.1.${BUILD_NUMBER}"
        PREPROD_IMAGE_TAG = "preprod-mbooking-ms-v.1.${BUILD_NUMBER}"
    }
        ...
        stage('Build and Test for Dev') {
            ...
            stages {
                ...
                stage('Build & Tag Docker Image') {
                    steps {
                        echo "Building Docker Image: ${ECR_URL}/mbooking-ms:${DEV_IMAGE_TAG}"
                        sh "docker build -t ${ECR_URL}/mbooking-ms:${DEV_IMAGE_TAG} ."
                ...
                stage('Push Docker Image to Amazon ECR') {
                    steps {
                        echo "Pushing Docker Image to ECR: ${ECR_URL}/mbooking-ms:${DEV_IMAGE_TAG}"
                        withDockerRegistry([credentialsId: 'ecr:ap-south-1:ecr-credentials', url: "https://${ECR_URL}"]) {
                            sh "docker push ${ECR_URL}/mbooking-ms:${DEV_IMAGE_TAG}"
                        }
            ...
            steps {
                script {
                    ...
                    if (configMapChanged == 0) {
                        ...
                        sh """
                            kubectl --kubeconfig=/var/lib/jenkins/.kube/config rollout restart deployment dev-mbooking-ms-deployment -n dev
                        """
                    } else {
        ...
        stage('Deploy app to prod env') {
            ...
            steps {
                script {
                    ...
                    sh """
                        sed -i 's|<latest>|prod-mbooking-ms-v.1.${BUILD_NUMBER}|g' ${yamlFile}
                        cat ${yamlFile} | grep prod-mbooking-ms-v.1.${BUILD_NUMBER} || echo "Replacement failed in ${yamlFile}"
                    """
                    sh """
